    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-3.5 py-2.5 text-xs rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors"
          />
        </div>
        <button
          type="button"
      ...
      {loading && members.length === 0 ? (
        <div className="flex items-center justify-center py-16 text-slate-400 text-xs gap-2">
          <Loader2 className="w-4 h-4 animate-spin text-primary-600" /> Loading members…
        </div>
      ) : members.length === 0 ? (
        <EmptyState
          icon={Users}
          title={search ? "No matching members found" : "No loyalty club members yet"}
          message={
            search
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {members.map((m) => {
            ...
            return (
              <Card
                key={m.id}
                className="flex flex-col justify-between space-y-4 hover:border-primary-200 transition-colors"
              >
                {/* Header */}
                  ...
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${
                        isRewardReady ? "bg-emerald-500" : "bg-primary-500"
                      }`}
